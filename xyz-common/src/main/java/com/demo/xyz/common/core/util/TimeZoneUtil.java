/*
 * 文件名：DateTimeZoneTest.java
 * 版权：Copyright by www.cloudminds.com
 * 描述：
 * 修改人：alex.xu
 * 修改时间：2020年2月17日
 * 跟踪单号：
 ...
 *
 * @author https://blog.csdn.net/zhanglu0223/article/details/47378407
 * @version 2020年2月17日
 * @see TimeZoneUtil
 * @since
 */
public class TimeZoneUtil {
    ...
    public static final String TIME_ZONE_GMT_0 = "GMT0";
     ...
     *
     * @param time           时间字符串
     * @param pattern        pattern 格式 "yyyy-MM-dd HH:mm:ss"
    ...
    public static String timeZoneTransfer(String time, String pattern, String nowTimeZone, String targetTimeZone) {
        ...
        sdf.setTimeZone(TimeZone.getTimeZone("GMT" + nowTimeZone));
        Date date;
        try {
            date = sdf.parse(time);
        } catch (ParseException e) {
    ...
    /**
     * 根据租户 ZoneOffset 获取时区字符串
     *
     * @param offset(小时)
     * @return
     */
    public static String getTenantZoneStrByOffset(Integer offset) {
        // 默认为东八区
        if (offset == null) {
            offset = 8;
        }
    ...
    /**
     * 根据租户 分钟数 ZoneOffset 获取时区字符串
     *
     * @param offset(分钟)
     * @return
     */
    public static String getTenantZoneStrByMinutesOffset(Integer offset) {
        // 默认为东八区
        if (offset == null) {
            offset = 480;
        }
        ...
        return StrUtil.format("GMT{}{}:{}", pre, String.format("%02d", Math.abs(hours)), String.format("%02d", Math.abs(minutes)));
    }
     ...
     * @param nowTimeZone    eg:GMT+08:00，GMT+00:00，GMT-01:00 等等
     * @param targetTimeZone 同nowTimeZone
     * @return
     */
    public static String minutesTimeZoneTransfer(String time, String pattern, String nowTimeZone, String targetTimeZone) {
        if (StringUtils.isBlank(time)) {
            return "";
        }
        SimpleDateFormat sdf = new SimpleDateFormat(pattern);
        sdf.setTimeZone(TimeZone.getTimeZone(nowTimeZone));
        Date date;
        try {
            date = sdf.parse(time);
        } catch (ParseException e) {
